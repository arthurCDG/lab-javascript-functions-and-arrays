...
let arrayTest = [6, 12, 'miami', 1, 'barca', '200', 'lisboa', 8, false];
...
function uniquifyArray(array) {
  ...
  if (array.length === 0) {
    ...
    uniquifiedArray.push(array[0]);

    for (let i = 1; i < array.length; i++) {
      for (let j = 0; j < uniquifiedArray.length; j++) {
        if (array[i] !== uniquifiedArray[j]) {
